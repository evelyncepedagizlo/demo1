...
class AccountJournal(models.Model):
    ...
    third_name = fields.Char(string='Nombre')
    third_account_id = fields.Many2one('account.account', string='Cuenta Destino')
    ...
    def _compute_check_analytic(self):
        for l in self:
            if l.third_account_id.analytic_account==True:
                l.check_analytic=True
            else:
                l.check_analytic=False
   ...
   #                          'payment_id': payment.id,
   #                          'analytic_account_id':linea.cuenta_analitica.id  ,
   #                      }),

   #                      listaMovimientos.add(tupla)
