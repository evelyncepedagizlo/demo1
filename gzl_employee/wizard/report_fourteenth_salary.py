...
class ReportFourteenthSalary(models.TransientModel):
    _name = "report.fourteenth.salary"
    ...
    basic_salary = fields.Float(string='Salario Básico', default=400, required=True,config_parameter="gzl_employee.sueldo_basico")





    def report_fourteenth_salary_data(self):
        dct={}
        lis=[]
        cont=0
        ...
        for emp in hr_employee:
            ...
            if contract:
                today = date.today()
                if self.date in ('01','02'):
                    date_start = str(int(self.year_date)-1)+'-03-01'
                else:
                    date_start = str(int(self.year_date))+'-03-01'
                ...
                dct = { 'nro': cont,
                        ...
                        'valor_a_pagar': round((self.basic_salary/12/30)*worked_day,2) if self.basic_salary else False 
                        } 
                lis.append(dct)
        return lis
    ...
    def xslx_body(self,workbook,name):
        ...
        data = self.report_fourteenth_salary_data()
