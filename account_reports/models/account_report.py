...
class AccountReport(models.AbstractModel):
    ...
    def _create_hierarchy(self, lines, options):
        ...
        def merge_columns(columns):
            return [('0.0%' if any(i != '' for i in x) else '') if any(isinstance(i, str) for i in x) else sum(x) for x in zip(*columns)]
    ...
    def _get_options(self, previous_options=None):
        ...
        if previous_options:
            lista=previous_options and previous_options.get('unfolded_lines')     or []
            lista= lista + previous_options.get('unfolded_lines_nuevas',[]) 
        else:
            lista=[]
        options = {
            'unfolded_lines': lista,
        }
