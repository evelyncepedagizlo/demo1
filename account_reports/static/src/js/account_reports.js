...
var accountReportsWidget = AbstractAction.extend({
    ...
    render_searchview_buttons: function() {
        ...
        this.$searchview_buttons.find('.js_niveles1_filtro').click(function (event) {
            var option_value = $(this).data('filter');
            self.report_options[option_value] = !self.report_options[option_value];
            self.unfold_all(true);
            self.reload();
        });
        
        
        this.$searchview_buttons.find('.js_niveles2_filtro').click(function (event) {
            var option_value = $(this).data('filter');
            self.unfold_nivel(true,2);
            self.reload();

        });
        
        this.$searchview_buttons.find('.js_niveles3_filtro').click(function (event) {
            var option_value = $(this).data('filter');
            self.unfold_nivel(true,3);
            self.reload();

            
        });

        this.$searchview_buttons.find('.js_niveles4_filtro').click(function (event) {
            var option_value = $(this).data('filter');
            self.unfold_nivel(true,4);
            self.reload();
    ...
    unfold_nivel: function(bool,cantidad_nivel) {
        var self = this;
        var lines = this.$el.find('.js_account_report_foldable');
        self.report_options.unfolded_lines_nuevas = [];
        if (bool) {
            _.each(lines, function(el) {
                
            let cadena= $(el).data('id');
            
            if (typeof cadena =='string'){
            
                var cadena_token = cadena.substring(10,14).trim();
                
                var tamanio_cadena= cadena_token.length;
                
                if(  $(el).data('unfolded') == 'False' &&  tamanio_cadena==cantidad_nivel ){
                    self.report_options.unfolded_lines_nuevas.push($(el).data('id'));
                    $(el).click();              
                }
            }
            });
